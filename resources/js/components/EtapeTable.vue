...
const handleGlobalEsc = (e: KeyboardEvent) => {
    if (e.key === 'Escape' && editingId.value !== null) cancelEdit();
    if (e.key === 'Enter'  && editingId.value !== null) handleSave();
};
...
/**
 * Kumpulkan semua branch_id di bawah sebuah regional
 * (baik yang lewat area maupun yang langsung di bawah regional).
 */
const getAllBranchesOfRegional = (regional: any): any[] => {
    const fromAreas: any[] = (regional.areas ?? []).flatMap((a: any) => a.branches ?? []);
...
const handleRegionalPIC = (regionalId: number, userId: number) => {
    ...
    router.post(
        ...
        {
            preserveScroll: true,
            onError: (errors) => { console.error('Bulk update error:', errors); },
        },
    );
};
...
<template>
    <div class="overflow-x-auto rounded-md border">
        <Table>
            <TableHeader>
                <TableRow>
                    <TableHead class="w-[220px] min-w-[220px] font-bold">
                    ...
                    <TableHead class="w-40 min-w-40 text-center font-bold">
                        <div class="flex flex-col">
                            ...
                            <span class="text-xs font-normal text-muted-foreground">CLQH / BC</span>
                        </div>
                    </TableHead>
                    ...
                    <TableHead class="w-40 min-w-40 text-center font-bold">
                        <div class="flex flex-col">
                            <span>Komitmen Etape</span>
                            <span class="text-xs font-normal text-muted-foreground">RLQH / BM</span>
                        </div>
                    </TableHead>

                    <TableHead class="w-[180px] min-w-[180px] text-right font-bold">
                        <div class="flex flex-col items-end">
                            <span>Outstanding</span>
                            <span class="text-xs font-normal text-muted-foreground">Dalam Juta</span>
                        </div>
                    </TableHead>
            ...
            <TableBody>
                <!-- Empty state -->
                <TableRow v-if="!props.performance_etapes || props.performance_etapes.length === 0">
                ...
                <!-- Iterasi Regional -->
                <template v-for="regional in props.performance_etapes" :key="regional.id">

                    <!-- ===== BARIS REGIONAL ===== -->
                    <TableRow class="bg-primary/10 hover:bg-primary/20 border-t-2 border-primary/20">
                        <!-- Nama Regional -->
                        <TableCell class="font-bold text-base">
                            {{ regional.name }}
                        </TableCell>

                        <!-- PIC Regional (dropdown untuk set semua cabang) -->
                        <TableCell>
                            <div class="flex items-center gap-2">
                                <Badge
                                    v-if="regional.pic"
                                    :class="getBadgeColor(regional.pic.color?.name ?? 'Abu-Abu')"
                                >
                                    {{ regional.pic.name }}
                                </Badge>
                                <DropdownMenu>
                                    <DropdownMenuTrigger as-child>
                                        <Button variant="outline" size="sm" class="h-8 gap-2">
                                            <span class="hidden sm:inline">Pilih PIC</span>
                                        </Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent align="end" class="w-56">
                                        <DropdownMenuLabel>Pilih PIC untuk Regional</DropdownMenuLabel>
                                        <DropdownMenuSeparator />
                                        <DropdownMenuItem
                                            v-for="user in props.users"
                                            :key="user.id"
                                            @click="handleRegionalPIC(regional.id, user.id)"
                        ...
                        <!-- Total prognosa regional -->
                        <TableCell class="text-right font-mono font-bold tabular-nums text-base">
...
<!--                                {{ area.branches?.length || 0 }} cabang-->
                        ...
                        >
                            ...
                            <!-- PIC cabang (inline edit) -->
                            <TableCell @click="startEdit(branch)">
                                <div v-if="editingId === branch.branch_id" @click.stop class="w-full">
                                    <Select v-model="editForm.user_id" :disabled="isProcessing">
                                ...
                                <div v-else class="cursor-pointer">
                                    <Badge
                                        v-if="branch.user"
                                        :class="getBadgeColor(branch.user.color?.name ?? 'Abu-Abu')"
                                    >
                                        {{ branch.user_name }}
                                    </Badge>
                            ...
                            <TableCell @click="startEdit(branch)">
                                <div v-if="editingId === branch.branch_id" @click.stop class="w-full">
                                    <Select v-model="editForm.komitmen_etape_bc_id" :disabled="isProcessing">
                                        <SelectTrigger class="h-9 w-full">
                                            <SelectValue placeholder="Pilih Komitmen" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem
                                                v-for="komitmen in props.categories.komitmen_etape_bc"
                                                :key="komitmen.id"
                                                :value="komitmen.id"
                                            >
                                                {{ komitmen.name }}
                                ...
                                <div v-else class="cursor-pointer">
                                    <Badge
                                        ...
                                        :class="getBadgeColor(branch.komitmen_etape_bc.color?.name ?? 'Abu-Abu')"
                                    >
                            ...
                            <TableCell @click="startEdit(branch)">
                                <div v-if="editingId === branch.branch_id" @click.stop class="w-full">
                                    <Select v-model="editForm.komitmen_etape_bm_id" :disabled="isProcessing">
                                        <SelectTrigger class="h-9 w-full">
                                            <SelectValue placeholder="Pilih Komitmen" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem
                                                v-for="komitmen in props.categories.komitmen_etape_bm"
                                                :key="komitmen.id"
                                                :value="komitmen.id"
                                            >
                                                {{ komitmen.name }}
                                ...
                                <div v-else class="cursor-pointer">
                                    <Badge
                                        ...
                                        :class="getBadgeColor(branch.komitmen_etape_bm.color?.name ?? 'Abu-Abu')"
                                    >
                            ...
                            <!-- Prognosa -->
                            <TableCell class="text-right" @click="startEdit(branch)">
                                <div v-if="editingId === branch.branch_id" @click.stop class="w-full">
                                    <Input
                                ...
                                <div v-else class="cursor-pointer font-mono text-sm tabular-nums">
                                    {{ formatAngkaPrognosa(branch.prognosa_akhir_bulan) }}
                                </div>
                            </TableCell>
                            ...
                            <TableCell>
                                ...
                                <div v-else @click="startEdit(branch)" class="cursor-pointer">
                                    <div
                                        v-if="branch.kendala"
                    ...
                    <!-- ===== CABANG LANGSUNG DI BAWAH REGIONAL (tanpa area) ===== -->
                    <TableRow
                    ...
                    >
                        ...
                        <TableCell
                            class="cursor-pointer pl-8 font-medium italic text-muted-foreground"
                            @click="startEdit(branch)"
                        >
                            {{ branch.branch_name }}
                        </TableCell>
            ...
            <TableFooter v-if="props.nasional">
                <TableRow>
                    <TableCell class="text-lg font-bold">NASIONAL</TableCell>
                    <TableCell></TableCell>
                    <TableCell></TableCell>
                    <TableCell></TableCell>
                    <TableCell class="text-right font-mono text-lg font-bold tabular-nums">
